...
bool FreeRTOSRuntimeStats::generate() {
    ...
    for (size_t i {}; i < num_tasks; ++i) {
        current_runtimes_.push_back(std::make_pair(
            task_data_[i].xHandle, (task_data_[i].ulRunTimeCounter - last_runtimes_[task_data_[i].xHandle]) * 100.f / (total_runtime - last_total_runtime_)));
...
bool FreeRTOSRuntimeStats::print() {
    ...
    for (auto& e : current_runtimes_) {
        ...
        Serial.printf("%s%s%s%.2f%% \t%lu byte\r\n", name, tabs, space, e.second, stack_free);
    }
